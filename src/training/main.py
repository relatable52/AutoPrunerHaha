...
def get_args():
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="../replication_package/model/rq1/autopruner/wala.pth",
        help="Path to checkpoint (for test only)",
    )
    parser.add_argument(
...
def main():
    ...
    train_dataset = FinetunedDataset(config, "train", args.model, args.loss_fn)
    test_dataset = FinetunedDataset(config, "test", args.model, args.loss_fn)
    ...
    if args.feature == 2:
        input_size = get_emb_size(args.model)
